...
public class ChatServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			List<Chat> chatList;
			chatList = ChatDAO.getChatList(last);
			...
			for (int i=0; i<chatList.size();i++)
			{
				...
				if(current_name != null && current_name.equals(chat.getName()))
					mine = "mine";
			
				JSONObject jobj = new JSONObject();
				jobj.put("name", chat.getName());
				jobj.put("message", chat.getMessage());
				jobj.put("time", chat.getTimeString());
				jobj.put("id", chat.getId());
				jobj.put("mine",  mine);
				jsonList.add(jobj);
				last=chat.getId();
			}
